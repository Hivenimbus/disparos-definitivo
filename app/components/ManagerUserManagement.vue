  ...
  <section class="bg-white rounded-lg shadow p-6">
    <div class="flex items-center justify-between mb-6">
      <div class="flex items-center space-x-2">
        ...
        <div>
          <h2 class="text-xl font-semibold text-gray-800">Usuários da Empresa</h2>
          <p class="text-sm text-gray-500" v-if="company">
            {{ company.nome || 'Empresa' }} — {{ company.usuariosAtuais }}/{{ company.maxUsuarios }} usuários
          </p>
        </div>
      </div>
    ...
    <div class="grid grid-cols-4 gap-4 mb-6">
      ...
      <div class="bg-gray-100 rounded-lg p-6 text-center">
        <div class="text-4xl font-bold text-red-600 mb-2">{{ inactiveUsers }}</div>
        <div class="text-gray-600 text-sm">Desativados</div>
      </div>
      <div class="bg-gray-100 rounded-lg p-6 text-center">
        <div class="text-4xl font-bold text-purple-600 mb-2">
          <span v-if="company">{{ company.usuariosAtuais }}/{{ company.maxUsuarios }}</span>
          <span v-else>--</span>
        </div>
        <div class="text-gray-600 text-sm">Capacidade</div>
      </div>
    </div>
            ...
            >
              ...
              <td class="px-4 py-3">
                <span
                  ...
                  :class="roleBadgeClasses[user.role] || roleBadgeClasses.usuario"
                >
              ...
              <td class="px-4 py-3">
                <div class="flex items-center space-x-2">
                  ...
                  <button
                    ...
                    :disabled="user.id === authUser.value?.id"
                    class="text-red-600 hover:text-red-800 transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
                    :title="user.id === authUser.value?.id ? 'Você não pode excluir sua própria conta.' : ''"
                  >
                    <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <div class="bg-white rounded-lg shadow-xl max-w-md w-full p-6">
        ...
        <div class="space-y-4 mb-6">
          ...
          <div>
            ...
            <div class="px-4 py-2 border border-dashed border-gray-300 rounded-lg bg-gray-50 text-gray-600">
              {{ vencimentoDisplay }}
            </div>
            <p class="text-xs text-gray-500 mt-1">
              Definida pelo administrador. Entre em contato com o suporte caso precise alterar.
            </p>
          </div>
...
const baseEndpoint = '/api/manager/users'
...
} = await useAsyncData<ManagerUsersResponse>('manager-users-list', async () => {
  return await $fetch(baseEndpoint, {
    headers: requestHeaders
  })
})

const users = computed<AdminUserItem[]>(() => managerData.value?.users ?? [])
const company = computed<CompanyItem | null>(() => managerData.value?.company || null)

const totalUsers = computed(() => users.value.length)
const activeUsers = computed(() => users.value.filter((u) => u.status === 'ativo').length)
const inactiveUsers = computed(() => users.value.filter((u) => u.status === 'desativado').length)
...
const loadingTable = computed(() => usersPending.value)
const listError = computed(() => !!usersError.value)

const vencimentoDisplay = computed(() => {
  const date = isEditMode.value ? userForm.value.dataVencimento : company.value?.dataVencimento || userForm.value.dataVencimento
  if (!date) {
...
const openAddModal = () => {
  ...
  if (!company.value) {
    toast.error('Não foi possível identificar a empresa. Atualize a página e tente novamente.')
    return
  }
  if (company.value.usuariosAtuais >= company.value.maxUsuarios) {
    toast.error('Limite de usuários atingido. Entre em contato com o suporte para solicitar aumento de capacidade.')
    return
  }
  resetForm()
  userForm.value.dataVencimento = company.value.dataVencimento || ''
...
const ensureCapacity = () => {
  if (!company.value) {
    formError.value = 'Não foi possível identificar os limites da empresa.'
    return false
  }
  if (!isEditMode.value && company.value.usuariosAtuais >= company.value.maxUsuarios) {
    formError.value = `Limite de usuários atingido para a empresa ${company.value.nome || ''}.`
    return false
  }
  return true
}
...
const saveUser = async () => {
  ...
  try {
    ...
    let response: ManagerUserResponse

    if (isEditMode.value && userForm.value.id) {
      response = await $fetch<ManagerUserResponse>(`${baseEndpoint}/${userForm.value.id}`, {
        method: 'PUT',
        body: payload
      })
    } else {
      response = await $fetch<ManagerUserResponse>(baseEndpoint, {
        method: 'POST',
        body: payload
      })
    }
    ...
    const message = (error as { statusMessage?: string })?.statusMessage || 'Erro ao salvar usuário.'
...
const openDeleteModal = (user: AdminUserItem) => {
  if (user.id === authUser.value?.id) {
    toast.error('Você não pode excluir sua própria conta. Solicite ao suporte.')
    return
  }
  deleteError.value = ''
...
const confirmDelete = async () => {
  ...
  if (userToDelete.value.id === authUser.value?.id) {
  ...
  try {
    await $fetch(`${baseEndpoint}/${userToDelete.value.id}`, {
      method: 'DELETE'
    })
    await refreshManagerData()
    closeDeleteModal()
  } catch (error) {
    deleteError.value = (error as { statusMessage?: string })?.statusMessage || 'Erro ao excluir usuário.'
  } finally {
...
const handleStatusChange = async (user: AdminUserItem, nextStatus: UiStatus) => {
  ...
  try {
    await $fetch<ManagerUserResponse>(`${baseEndpoint}/${user.id}`, {
      method: 'PUT',
      body: {
        status: nextStatus
      }
