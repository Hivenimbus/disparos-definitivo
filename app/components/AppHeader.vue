<template>
  <div class="sticky top-0 z-50">
    <header class="bg-white shadow-sm border-b border-gray-200">
      <div class="px-4 py-4 flex items-center justify-between md:px-8">
        <div class="flex items-center space-x-4 md:space-x-16">
          <NuxtLink to="/" class="block h-10 md:h-12">
            <img
              src="/Logo.png"
          ...
          <nav class="hidden md:flex items-center space-x-8">
            <template v-for="item in availableMenuItems" :key="item.name">
              ...
              >
                ...
                <svg v-if="item.icon === 'IconUpdates'" xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div class="flex items-center space-x-4">
          ...
          >
            <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16m-7 6h7" v-if="!isMobileMenuOpen" />
        ...
        <nav class="flex flex-col py-4">
          <template v-for="item in availableMenuItems" :key="item.name">
            <NuxtLink
              ...
              class="flex items-center space-x-3 px-6 py-3 text-base font-medium transition-colors"
              :class="$route.path === item.path ? 'text-blue-600 bg-blue-50' : 'text-gray-700 hover:bg-gray-50 hover:text-blue-600'"
            >
              <svg v-if="item.icon === 'IconDashboard'" xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 12l3-3 3 3 4-4M8 21l4-4 4 4M3 4h18M4 4h16v12a1 1 0 01-1 1H5a1 1 0 01-1-1V4z" />
              </svg>
            ...
            <button
              v-else
              @click="toggleConfigPanel(); isMobileMenuOpen = false"
              class="flex items-center space-x-3 px-6 py-3 text-base font-medium transition-colors w-full text-left"
              :class="isConfigOpen ? 'text-blue-600 bg-blue-50' : 'text-gray-700 hover:bg-gray-50 hover:text-blue-600'"
            >
              <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z" />
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
          ...
          <div class="px-6 py-3 flex items-center space-x-3">
            ...
            <span class="text-base font-medium text-gray-800">{{ displayName }}</span>
          </div>
          ...
          <NuxtLink
            ...
            :to="option.path"
            @click="isMobileMenuOpen = false"
            class="block px-6 py-3 text-base text-gray-700 hover:bg-gray-50 transition-colors"
          >
            {{ option.name }}
          </NuxtLink>
          <button
            type="button"
            @click="handleLogout"
            class="w-full text-left px-6 py-3 text-base text-red-600 hover:bg-red-50 transition-colors"
          >
            Sair
          </button>
        </nav>
